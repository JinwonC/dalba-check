...
// 임의 스프레드시트/범위 조회 (점검용)
export async function peek(sid, tabAndRange) {
  const tk = await token();
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${enc(tabAndRange)}`;
  const res = await fetch(url, { headers: { Authorization: `Bearer ${tk}` } });
  const j = await res.json().catch(() => ({}));
  if (!res.ok) return { error: `${res.status}: ${JSON.stringify(j).slice(0, 250)}` };
  return { values: j.values || [] };
}
export async function peekTabs(sid) {
  const tk = await token();
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${sid}?fields=${enc('sheets.properties.title')}`;
  const res = await fetch(url, { headers: { Authorization: `Bearer ${tk}` } });
  const j = await res.json().catch(() => ({}));
  if (!res.ok) return { error: `${res.status}: ${JSON.stringify(j).slice(0, 250)}` };
  return { tabs: (j.sheets || []).map((s) => s.properties && s.properties.title) };
}
